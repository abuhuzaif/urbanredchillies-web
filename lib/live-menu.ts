...
export async function getLiveMenu(): Promise<LiveSection[]> {
  ...
  const { data: items, error: itemsErr } = await supabase
    .from("menu_items")
    .select(
      "id, category_id, name, name_ar, price, image_url, has_variants, is_available, is_today_offer, offer_price"
    )
    ...
    const catItems: LiveItem[] = items
      ...
      .map((i) => {
        ...
        return {
          ...
          isTodayOffer: (i.is_today_offer as boolean) ?? false,
...
export async function getAnnouncement(): Promise<Announcement | null> {
  const { data, error } = await supabase
    ...
    .select("message, text_color, font_style, is_active")
    .eq("id", "main")
    .maybeSingle();
